...
void akjet_efficiency_Wplus_minus_H125ToCC_WLNu_powheg_13_1L() {
...
Float_t MET_Pt[50];
Float_t htJet30[50];
Float_t nAddJetsFSRsub302p5_puid;

int isZmm,isZee,isWmunu,isWenu;
int controlSample;
...
UInt_t  nElectron, nMuon;
 ...
 // TChain *chain = new TChain("tree");
   for (int fileNum=1;fileNum < 42;fileNum++) {
      t->AddFile(Form("output_WminusH125ToCC_WLNu_powheg_%d.root", fileNum));
//std::cout<<"File number = "<<fileNum<<std::endl; 
 }

 for (int fileNum=1;fileNum < 27;fileNum++) {
      t->AddFile(Form("output_WplusH125ToCC_WLNu_powheg_%d.root", fileNum));
//std::cout<<"File number = "<<fileNum<<std::endl; 
 }
//
  ...
  t->SetBranchAddress("twoResolvedJets",&twoResolvedJets);
        ...
        t->SetBranchAddress("isZee",&isZee);
         ...
         t->SetBranchAddress("isWmunu",&isWmunu); 
	t->SetBranchAddress("isWenu",&isWenu);
 t->SetBranchAddress("nAddJetsFSRsub302p5_puid",&nAddJetsFSRsub302p5_puid);
  t->SetBranchAddress("MET_Pt",MET_Pt);
        t->SetBranchAddress("htJet30",&htJet30);
...
//int ak4_selected_true=0;
 ...
 int NumberOfEvents =70085;
//	int NumberOfEvents =408517;
...
/////////////Denominator for resolved jet topology efficiency////////////////////
                ...
                if(GenHiggs.Pt()>0&&(isWmunu || isWenu)&&JetPt_1[0]>20 &&V_pt[0]>100&&(MET_Pt[0]/sqrt(htJet30[0]))>4&&GenHiggs.Eta()<2.4&&GenHiggs.Eta()>-2.4 &&JetPt_2[0]>20){
...
/////////////Denominator for boosted  jet topology efficiency////////////////////
 if(GenHiggs.Pt()>0&&(isWmunu || isWenu)&&(MET_Pt[0]/sqrt(htJet30[0]))>4&&V_pt[0]>100&&GenHiggs.Eta()<2.4&&GenHiggs.Eta()>-2.4){
	...
	}//end of for loop genparticles

		if(LeadAK15.Pt()>0&&GenHiggs.Pt()>0&&LeadAK15.DeltaR(GenHiggs)<0.1){
			genhiggs_with_ak15->Fill(GenHiggs.Pt());
									}

		if(LeadAK8.Pt()>0&&GenHiggs.Pt()>0&&LeadAK8.DeltaR(GenHiggs)<0.1){
			genhiggs_with_ak8->Fill(GenHiggs.Pt());
									}	


/////////////Numerator  for boosted jet topology efficiency////////////////////
	  if(LeadAK15.Pt()>0&&GenHiggs.Pt()>0&&(isWmunu || isWenu)&&V_pt[0]>100&&GenHiggs.Eta()<2.4&&(MET_Pt[0]/sqrt(htJet30[0]))>4&&GenHiggs.Eta()>-2.4 &&LeadAK15.DeltaR(GenHiggs)<0.3){
                genhiggs_with_ak15_deltaR_0_3->Fill(GenHiggs.Pt());
                genhiggs_with_ak15_deltaR_0_3_varbin->Fill(GenHiggs.Pt());

                 }

        if(LeadAK8.Pt()>0&&GenHiggs.Pt()>0&&(isWmunu || isWenu)&&GenHiggs.Eta()<2.4&&V_pt[0]>100&&(MET_Pt[0]/sqrt(htJet30[0]))>4&&GenHiggs.Eta()>-2.4 &&LeadAK8.DeltaR(GenHiggs)<0.3){
...
/////////////Numerator  for resolved jet topology efficiency////////////////////
            ...
            if(Hcc.Pt()>0&&
 (isWmunu || isWenu)&&GenHiggs.Eta()<2.4&&GenHiggs.Eta()>-2.4 &&(MET_Pt[0]/sqrt(htJet30[0]))>4&&JetPt_1[0]>20 && JetPt_2[0]>20 &&H_mass[0]>50 && H_mass[0]<200 
...
if((isWmunu || isWenu) && JetPt_1[0]>25 && JetPt_2[0]>25 && (MET_Pt[0]/sqrt(htJet30[0]))>4 && H_pt[0]>100 &&
DeepJet_CvsL_1[0]>0.225 && DeepJet_CvsB_1[0]>0.40 && DeepJet_CvsL_2[0]>0.00 && DeepJet_CvsB_2[0]>0.0 &&
 controlSample==0 && H_mass[0]>50 && H_mass[0]<200 && V_pt[0]>100 && HVdPhi[0]>2.5 && nAddJetsFSRsub302p5_puid<1.5)                                  {
...
///Numerator for lost of signal calculation//////////////////
...
TLegend* leg666 = new TLegend(0.1,0.76,0.4,0.95);
        ...
        leg666->Draw("same");
   canvas50->SaveAs("efficiency_genhiggs_1L_channel.pdf");
                delete canvas50;

/*
 TCanvas *canvas51 = new TCanvas("canvas51732567", "", 1900, 1100);
                canvas51->cd();
 canvas51->SetFillStyle(1001);
   canvas51->SetFillColor(kWhite);
                genhiggs_with_ak4_nosel->GetYaxis()->SetTitle("efficiency");
                genhiggs_with_ak4_nosel->GetYaxis()->SetTitleSize(0.05);
                genhiggs_with_ak4_nosel->GetYaxis()->SetTitleOffset(0.80);
                genhiggs_with_ak4_nosel->GetXaxis()->SetTitle("p_{T}, GeV");
                genhiggs_with_ak4_nosel->GetXaxis()->SetTitleSize(0.05);
                genhiggs_with_ak4_nosel->GetXaxis()->SetTitleOffset(0.90);
                genhiggs_with_ak4_nosel->SetLineWidth(2);
genhiggs_with_ak4_nosel->GetYaxis()->SetRangeUser(0., 1.);
genhiggsall->SetStats(0);
genhiggsall->Sumw2();
  genhiggs_with_ak4_nosel->SetLineColor(kOrange);
 genhiggs_with_ak4_nosel->SetStats(0);
 genhiggs_with_ak4_nosel->Sumw2();
 genhiggs_with_ak4_nosel->Divide(genhiggsall);
     genhiggs_with_ak4_nosel->Draw("E");

TLegend* leg51 = new TLegend(0.1,0.76,0.4,0.95);
       leg51->AddEntry(genhiggs_with_ak4_nosel,"with resolved if no cut,  #deltaR(H_{True},H_{AK4Jets})<0.1");
        leg51->SetTextSize(0.03);
        leg51->SetTextFont(42);
        leg51->SetBorderSize(0);
        leg51->Draw("same");
   canvas51->SaveAs("efficiency_genhiggs_1L_ak4_nosel.pdf");
                delete canvas51;
...
cout<<"ak4_ak4_ak15 ="<<num_int_3<<endl;
  ...
  TLegend* leg4 = new TLegend(0.1,0.91,0.3,0.85);
   leg4->AddEntry(frac_ak4_ak15,"AK4 & AK15");
        ...
        leg4->Draw("same");
  c4->SaveAs("tgraphefficiency_ak4_ak15_1L_channel.pdf");
  delete c4;
...
cout<<"ak4_ak4_ak8 ="<<num_int_2<<endl;
  TLegend* leg61 = new TLegend(0.1,0.91,0.3,0.85);
   leg61->AddEntry(frac_ak4_ak8,"AK4 & AK8");
        ...
        leg61->Draw("same");
  c61->SaveAs("tgraphefficiency_ak4_ak8_1L_channel.pdf");
  delete c61;
...
cout<<"ak4_ak4_ak15 ="<<num_int<<endl;
  ...
  TLegend* leg65 = new TLegend(0.1,0.91,0.3,0.85);
   leg65->AddEntry(frac_ak4_ak15_ak4,"AK4 & AK15");
        ...
        leg65->Draw("same");
  c65->SaveAs("tgraphefficiency_ak4_ak15_vs_nosel_ak15_pt_1L_channel.pdf");
  delete c65;
...
TLegend* leg66 = new TLegend(0.1,0.91,0.3,0.85);
   leg66->AddEntry(frac_ak4_ak8_ak4,"AK4 & AK8");
        ...
        leg66->Draw("same");
  c66->SaveAs("tgraphefficiency_ak4_ak8_vs_nosel_ak8_pt_1L_channel.pdf");
  delete c66;
...
mg1->Draw("a5");
mg1->Draw("p");
 TLegend* leg64 = new TLegend(0.1,0.99,0.25,0.8);
        ...
        leg64->Draw("same");
  c64->SaveAs("tgraphefficiency_ak15_deltaR_0_3_varbin_1L_channel.pdf");
  delete c64;
